...
final class FingrUITests: XCTestCase {
    ...
    func testLaunchIsImmediatelyPlayableAndDemoCanReplay() {
        ...
        XCTAssertEqual(app.staticTexts["game.selectionRule"].label, "Keep 1 · 2+ players")
        ...
        XCTAssertEqual(readyBoard.height, app.frame.height, accuracy: 1, "The play surface must fill the screen, including safe areas")
        ...
        XCTAssertEqual(app.staticTexts["game.status"].label, "Waiting for players")
        XCTAssertTrue(surface.isHittable)
    }
    ...
    func testMultipleWinnersAndSavedCount() {
        var app = launch()
        openOptions(app)
        XCTAssertEqual(app.staticTexts["settings.winnerCount.value"].label, "Keep 1 · Needs 2+ players")
        app.buttons["settings.winnerCount.2"].tap()
        app.buttons["settings.done"].tap()
        XCTAssertEqual(app.staticTexts["game.selectionRule"].label, "Keep 2 · 3+ players")
        startDemo(app)
        XCTAssertTrue(app.staticTexts["game.winner"].waitForExistence(timeout: 10))
        XCTAssertEqual(app.staticTexts["game.winner"].label, "2 selected")
        XCTAssertTrue(app.descendants(matching: .any).matching(identifier: "game.selectedCount").firstMatch.label.contains("2 fingers selected"))
    ...
    func testSixPlayerTeamsAndPickerSettingsPersist() {
        ...
        app.buttons["game.again"].tap()
        XCTAssertEqual(app.staticTexts["game.selectionRule"].label, "2 balanced teams · Wait for 6 players")
        app.terminate()
        app = launch(reset: false)
        XCTAssertTrue(app.staticTexts["game.selectionRule"].waitForExistence(timeout: 5))
        XCTAssertEqual(app.staticTexts["game.selectionRule"].label, "2 balanced teams · Wait for 6 players")
    ...
    func testOpeningOptionsCancelsDemoAndSwipeDismissReturnsToGame() {
        ...
        let dismissed = XCTNSPredicateExpectation(predicate: NSPredicate(format: "exists == false"), object: app.buttons["settings.done"])
        XCTAssertEqual(XCTWaiter.wait(for: [dismissed], timeout: 5), .completed)
        XCTAssertTrue(app.staticTexts["game.question"].waitForExistence(timeout: 3))
        XCTAssertEqual(app.staticTexts["game.question"].label, "Who teams up?")
        XCTAssertEqual(app.staticTexts["game.status"].label, "Waiting for players")
    ...
    func testTinyDareAddsAChallengeWithoutChangingTheWinner() {
        ...
        app.buttons["settings.done"].tap()
        XCTAssertEqual(app.staticTexts["game.question"].label, "Who's taking the challenge?")
